...
const S={
  ...
  zh:{subjects:' 名受试者',rights:'聚合研究结果',secondaryUp:'次指标 ↑',secondaryDown:'次指标 ↓',
      empty:'该协议下没有这一类别的已评测模型。请换一个类别。',noResults:'无可显示的结果',
      tradeoffTitle:'检出率与可靠性',accuracyTitle:'各配置的准确率',
      tradeoffNote:'越靠左上越好。始终弃权也能得到零误激活，所以检出率必须和误激活率一起读。',
      accuracyNote:'受试者平均平衡准确率。误差线在有数据时表示描述性 95% 区间。',
      chanceLine:(c:number)=>'虚线标出本任务 '+c+'% 的随机水平。',chanceSee:'随机水平见协议说明。',
      chartLabel:(y:string,x:string)=>y+'与'+x+'图',tip:(n:string,v:string)=>n+'：'+v,
      tipTradeoff:(n:string,d:string,f:string)=>n+'：检出率 '+d+'，误激活率 '+f,
      axisFa:'空闲误激活率 →',axisDetect:'指令检出率 % ↑',axisBa:'平衡准确率 % ↑',axisOrder:'配置按图例顺序排列',
      results:' 结果',
      atOrBelow:(c:number)=>'不高于 '+c+'% 随机水平',reaches:(c:number)=>'区间触及 '+c+'% 随机水平',
      seed:(w:string,n:number,lo:string,hi:string,m:string)=>'单一种子——'+n+' 次运行中'+w+'（'+lo+'–'+hi+'%，均值 '+m+'%）',
      seedWord:{highest:'最高的一次',lowest:'最低的一次',middle:'居中的一次'} as Record<string,string>,
      abstained:(a:number,n:number)=>n+' 名参与者中有 '+a+' 名始终弃权',
      ...
      dlProtocol:'下载协议 · JSON ↓',original:'原始数据集 ↗',participants:' 名参与者',
      abstainNote:(n:number,a:number)=>n+' 名参与者中，有 '+a+' 名在校准数据上选择了始终弃权的阈值。他们的测试试次仍计入分母。',
      cohortOnly:'只公开队列级聚合结果。个体分数、记录与预测均保留在本地。',
      metric:{'Balanced accuracy':'平衡准确率','Command detection ≤3s':'指令检出率 ≤3 秒','Idle false activation':'空闲误激活率','Macro F1':'宏平均 F1'} as Record<string,string>},
}[LANG];
